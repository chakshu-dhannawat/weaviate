...
// flow (NeedsPreparationBarrier=true):
//
...
//
//   - A PREPARING task crashed before its local PreparationCompleteAck landed
...
//
// preparationCallbackFired and preparationAckEmitted maps must also stay empty for
...
// countingTaskLister is a minimal TaskLister that counts calls instead of
...
// on the ticker channel (or wakeCh) could fire after Close() had already
// returned, running provider callbacks and synchronous RAFT applies
// against a caller that believed shutdown was complete.
//
